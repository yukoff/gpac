...
static GF_Err FFDEC_AttachStream(GF_BaseDecoder *plug, GF_ESD *esd)
{
	...
	/*use std MPEG-4 st/oti*/
		...
		if (ffd->st==GF_STREAM_VISUAL) {
			...
			codec_id = AV_AV_CODEC_ID_DVD_SUBTITLE;
		}
		*codec = avcodec_find_decoder(codec_id);
	}
	/*should never happen*/
	...
	if (codec_id == AV_AV_CODEC_ID_RAWVIDEO) {
		(*ctx)->codec_id = AV_AV_CODEC_ID_RAWVIDEO;
...
redecode:
	...
	if ( ctx->codec_id == AV_AV_CODEC_ID_RAWVIDEO) {
